...
public class BillListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(mContext).inflate(R.layout.item_bill, parent, false);
        ...
        // 根据类型显示金额：收入显示正数，支出显示负数（带负号）
        double amount = bill.amount;
        String amountText;
        if (bill.type == BillInfo.BILL_TYPE_INCOME) {
            amountText = "+" + String.format("%.2f", amount) + "元";
        } else {
            amountText = "-" + String.format("%.2f", amount) + "元";
        }
        holder.tv_amount.setText(amountText);
